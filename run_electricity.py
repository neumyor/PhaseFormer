...
def main():
    ...
    for horizon in horizons:
        print(f"\n{'='*50}")
        print(f"开始Electricity实验: {lookback}->{horizon}")
        print(f"{'='*50}")
        ...
        print(f"使用配置: layers={best_config['layers']}, latent_dim={best_config['latent_dim']}, "
              f"routers={best_config['phase_num_routers']}, heads={best_config['phase_attn_heads']}, "
        ...
        logger_version = (
            ...
            f"-routers{model_config.phase_num_routers}{loss_suffix}"
        )
        ...
        summary_records.append({
            ...
            "heads": best_config['phase_attn_heads'],
    ...
    if summary_records:
        ...
        summary_csv = os.path.join("./log/training_results/PhaseFormer", f"summary_electricity_{ts}.csv")
        headers = [
            "dataset", "lookback", "horizon", "layers", "latent_dim", "routers", "heads",
            ...
            with open(summary_csv, "w", newline="") as f:
                ...
                for r in summary_records:
                    writer.writerow([
                        ...
                        r.get("layers"), r.get("latent_dim"), r.get("routers"), r.get("heads"),
                        r.get("learning_rate"), r.get("test_mae"), r.get("test_mse"), 
                        r.get("log_dir"),
                    ])
            print(f"✅ Electricity实验汇总已导出到: {summary_csv}")
        except Exception as e:
            print(f"❌ 导出CSV失败: {e}")
        ...
        print("\nElectricity实验结果汇总:")
        print("| Horizon | Layers | Latent Dim | Routers | Heads | Learning Rate | Test MAE | Test MSE |")
        print("|---------|--------|------------|---------|-------|---------------|----------|----------|")
        for r in summary_records:
            print(
                "| "
