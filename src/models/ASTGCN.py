...
class cheb_conv_withSAt(nn.Module):
    ...
    def __init__(self, K, cheb_polynomials, in_channels, out_channels):
        ...
        self.K = K
        self.cheb_polynomials = nn.ParameterList([
            nn.Parameter(poly, requires_grad=False) for poly in cheb_polynomials
        ])
...
def make_model(
    ...
    :return:
    """
    adj_mx, distance_mx = get_adjacency_matrix(adj_file_path, num_of_vertices, None)
...
class ASTGCN(nn.Module):
    def __init__(self, configs, *args, **kwargs):
        ...
        self.model = make_model(
            DEVICE="cpu",
            nb_block=configs.e_layers,
            in_channels=configs.enc_input_size,
            K=3,
            nb_chev_filter=configs.d_model,
            nb_time_filter=configs.d_model,
            time_strides=1,
            adj_file_path=configs.adj_file_path,
    ...
    def forward(self, x_enc, *args, **kwargs):
        # x_enc [B, LxC, N]
        x_enc = rearrange(x_enc, "b (l c) n -> b n c l", c=self.enc_input_size)
        return rearrange(self.model(x_enc), "b n l -> b l n")
