...
class TimeBase_PSI(DefaultPLModule):
    """
    TimeBase Period-Segment with Intermediate supervision (PSI) 模型：
    带中间监督信号的串行双维度降维时间序列预测模型
    
    策略特点：
    - 在PS基础上增加中间监督信号
    - 第一阶段（Period降维）后添加中间预测头
    - 中间预测损失与最终预测损失联合训练
    - 引入门控机制调节两阶段信息流
    - 自适应权重平衡中间损失与最终损失
    
    创新设计：
    1. 中间监督信号：在Period降维后产生中间预测
    2. 门控融合机制：控制第一阶段向第二阶段的信息传递
    3. 自适应损失权重：动态平衡两个监督信号的重要性
    4. 渐进式特征提取：先学习Period模式，再优化Segment模式
    ...
    def __init__(self, configs):
        super(TimeBase_PSI, self).__init__(configs)
        ...
        if not self.use_period_norm or not self.use_segment_norm:
            print(f"⚠️ PSI策略建议启用双归一化: use_period_norm=True, use_segment_norm=True")
    ...
    def _setup_intermediate_supervision_components(self):
        ...
        # 中间预测头：将Period降维后的特征直接映射到预测长度
        # 输入：(b, period_len, seg_num_x) -> 输出：(b, pred_len)
        if self.individual:
            ...
            for i in range(self.enc_in):
                ...
                self.intermediate_predictors.append(
                    nn.Sequential(
                        nn.Linear(self.period_len * self.seg_num_x, 128),  # 压缩特征
                        nn.ReLU(),
                        nn.Linear(128, 64),
                        nn.ReLU(),
    ...
    def _setup_gating_mechanism(self):
        ...
        gate_input_dim = self.period_len * self.seg_num_x * self.enc_in
    ...
    def _generate_intermediate_prediction(self, x_period_reduced, b, c):
        """生成中间预测"""
        if not self.use_intermediate_supervision:
            return None
            
        # x_period_reduced: (b, c, period_len, seg_num_x)
        if self.individual:
            intermediate_preds = []
            for i in range(self.enc_in):
                # 对每个变量单独处理
                var_data = x_period_reduced[:, i, :, :]  # (b, period_len, seg_num_x)
                var_data_flat = var_data.reshape(b, -1)  # (b, period_len * seg_num_x)
                # 通过中间预测头
            ...
            # 重塑为适合预测头的形状
            x_flat = x_period_reduced.reshape(b * c, -1)  # (b*c, period_len * seg_num_x)
    ...
    def _apply_gating(self, x_period_reduced, b, c):
        """应用门控机制"""
        if not self.use_gating:
            return x_period_reduced
            
        # 计算门控权重
        x_flat = x_period_reduced.reshape(b, -1)  # (b, c*period_len*seg_num_x)
        x_flat_unsqueezed = x_flat.unsqueeze(1)  # (b, 1, c*period_len*seg_num_x)
        ...
        x_gated = x_period_reduced * gate_weights
    ...
    def forward(self, x_enc, x_mark_enc=None, x_dec=None, x_mark_dec=None, *args, **kwargs):
        ...
        # 🆕 中间监督：生成第一阶段的预测
        intermediate_prediction = self._generate_intermediate_prediction(x_period_reduced, b, c)
        
        # 🆕 门控机制：控制信息流
        x_period_gated = self._apply_gating(x_period_reduced, b, c)
        
        # 阶段2: Period reduction后的period逆归一化 + segment归一化
        x_flat = x_period_gated.reshape(-1, self.period_len, self.seg_num_x)
