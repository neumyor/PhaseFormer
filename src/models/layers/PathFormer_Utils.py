...
# Replace einops with custom implementations
def rearrange(tensor, pattern, **axes_lengths):
    """Simplified rearrange implementation for specific patterns used in PathFormer"""
    if pattern == 'f -> b f d':
        # f: frequency dimension -> b: batch, f: frequency, d: feature
        b, d = axes_lengths['b'], axes_lengths['d']
        return tensor.unsqueeze(0).unsqueeze(-1).expand(b, -1, d)
    elif pattern == 'b f d -> b f () d':
        # Add singleton dimension
        return tensor.unsqueeze(2)
    elif pattern == 't -> () () t ()':
        # Add batch and feature dimensions
        return tensor.unsqueeze(0).unsqueeze(0).unsqueeze(-1)
    elif pattern == 'b f d -> b f () d':
        return tensor.unsqueeze(2)
    elif pattern == 'b f t d -> b t d':
        # This is a reduction operation, handle in reduce function
        return tensor
    elif pattern == 'b o t d -> b d o t':
        # Permute dimensions
...
def reduce(tensor, pattern, reduction):
    ...
    if pattern == 'b f t d -> b t d' and reduction == 'sum':
        ...
        raise NotImplementedError(f"Pattern {pattern} with {reduction} not implemented")
...
def repeat(tensor, pattern, **axes_lengths):
    ...
    if pattern == 'f -> b f d':
        b, d = axes_lengths['b'], axes_lengths['d']
        return tensor.unsqueeze(0).unsqueeze(-1).expand(b, -1, d)
    elif pattern == 'f t -> b f t d':
        b, d = axes_lengths['b'], axes_lengths['d']
        return tensor.unsqueeze(0).unsqueeze(-1).expand(b, -1, -1, d)
    elif pattern == 't f -> b t f d':
        b, d = axes_lengths['b'], axes_lengths['d']
        return tensor.unsqueeze(0).unsqueeze(-1).expand(b, -1, -1, d)
    else:
        raise NotImplementedError(f"Pattern {pattern} not implemented")
...
class FourierLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        b, t, d = x.shape
        # Convert to float32 for FFT operations (FFT doesn't support bfloat16)
    ...
    def dft_forward(self, x):
        ...
        # Convert to float32 for FFT operations if needed
        original_dtype = x.dtype
        if x.dtype == torch.bfloat16:
            x = x.float()
        ...
        result = torch.einsum('botd,btd->bod', [attn, x])
        
        # Convert back to original dtype if needed
        if original_dtype == torch.bfloat16:
            result = result.to(original_dtype)
            attn = attn.to(original_dtype)
        
        return result, rearrange(attn, 'b o t d -> b d o t')
