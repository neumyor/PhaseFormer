...
class spatialGCN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 2, 1, 3).reshape(
            (-1, num_of_vertices, in_channels)
        )  # (b*t,n,f_in)

        return F.relu(
            self.Theta(torch.matmul(self.sym_norm_Adj_matrix, x))
            .reshape((batch_size, num_of_timesteps, num_of_vertices, self.out_channels))
            .transpose(1, 2)
        )
...
class Spatial_Attention_layer(nn.Module):
    ...
    def forward(self, x):
        ...
        score = torch.matmul(x, x.transpose(1, 2)) / math.sqrt(
            in_channels
...
class spatialAttentionGCN(nn.Module):
    def __init__(self, sym_norm_Adj_matrix, in_channels, out_channels, dropout=0.0):
        ...
        self.sym_norm_Adj_matrix = nn.Parameter(
            sym_norm_Adj_matrix, requires_grad=False
        )  # (N, N)
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.Theta = nn.Linear(in_channels, out_channels, bias=False)
    ...
    def forward(self, x):
        ...
        spatial_attention = spatial_attention.reshape(
            (-1, num_of_vertices, num_of_vertices)
        )  # (b*T, n, n)

        return F.relu(
            self.Theta(torch.matmul(self.sym_norm_Adj_matrix.mul(spatial_attention), x))
            .reshape((batch_size, num_of_timesteps, num_of_vertices, self.out_channels))
            .transpose(1, 2)
        )
...
class spatialAttentionScaledGCN(nn.Module):
    ...
    def forward(self, x):
        ...
        spatial_attention = self.SAt(x) / math.sqrt(
            in_channels
        )  # scaled self attention: (batch, T, N, N)
...
class SpatialPositionalEncoding(nn.Module):
    def __init__(self, d_model, num_of_vertices, dropout, gcn=None, smooth_layer_num=0):
        ...
        self.dropout = nn.Dropout(p=dropout)
        self.embedding = torch.nn.Parameter(torch.randn((num_of_vertices, d_model)))
        self.gcn_smooth_layers = None
        if (gcn is not None) and (smooth_layer_num > 0):
            self.gcn_smooth_layers = nn.ModuleList(
                [gcn for _ in range(smooth_layer_num)]
            )
    ...
    def forward(self, x):
        ...
        embed = self.embedding.unsqueeze(0)  # (N, d_model)->(1,N,d_model)
...
class PositionWiseGCNFeedForward(nn.Module):
    def __init__(self, gcn, dropout=0.0):
...
class MultiHeadAttention(nn.Module):
    def __init__(self, nb_head, d_model, dropout=0.0):
...
class MultiHeadAttentionAwareTemporalContex_qc_kc(
    nn.Module
    ...
    def __init__(
        ...
        self.conv1Ds_aware_temporal_context = clones(
            nn.Conv2d(d_model, d_model, (1, kernel_size), padding=(0, self.padding)), 2
    ...
    def forward(
        ...
        if query_multi_segment and key_multi_segment:
            ...
            if self.w_length > 0:
                query_w, key_w = [
                    l(x.permute(0, 3, 1, 2))[:, :, :, : -self.padding]
                    .contiguous()
                    .view(nbatches, self.h, self.d_k, N, -1)
                    .permute(0, 3, 1, 4, 2)
                    for l, x in zip(
                        self.conv1Ds_aware_temporal_context,
                        (
                            query[:, :, : self.w_length, :],
                            key[:, :, : self.w_length, :],
                        ),
            ...
            if self.d_length > 0:
                query_d, key_d = [
                    ...
                    for l, x in zip(
                        ...
                        (
                            query[
                                :, :, self.w_length : self.w_length + self.d_length, :
                            ],
            ...
            query = (
                self.conv1Ds_aware_temporal_context[0](query.permute(0, 3, 1, 2))[
                    :, :, :, : -self.padding
                ]
                .contiguous()
                .view(nbatches, self.h, self.d_k, N, -1)
            ...
            if self.w_length > 0:
                key_w = (
                    self.conv1Ds_aware_temporal_context[1](
                        key[:, :, : self.w_length, :].permute(0, 3, 1, 2)
                    )[:, :, :, : -self.padding]
                    .contiguous()
                    .view(nbatches, self.h, self.d_k, N, -1)
                    .permute(0, 3, 1, 4, 2)
...
class MultiHeadAttentionAwareTemporalContex_q1d_k1d(
    nn.Module
...
class MultiHeadAttentionAwareTemporalContex_qc_k1d(
    nn.Module
    ...
    def __init__(
        ...
        self.query_conv1Ds_aware_temporal_context = nn.Conv2d(
            d_model, d_model, (1, kernel_size), padding=(0, self.causal_padding)
        )
        self.key_conv1Ds_aware_temporal_context = nn.Conv2d(
            d_model, d_model, (1, kernel_size), padding=(0, self.padding_1D)
        )
        self.dropout = nn.Dropout(p=dropout)
        self.w_length = num_of_weeks * points_per_hour
        self.d_length = num_of_days * points_per_hour
    ...
    def forward(
        ...
        if query_multi_segment and key_multi_segment:
            ...
            if self.w_length > 0:
                query_w = (
                    self.query_conv1Ds_aware_temporal_context(
                        query[:, :, : self.w_length, :].permute(0, 3, 1, 2)
                    )[:, :, :, : -self.causal_padding]
                    .contiguous()
                    .view(nbatches, self.h, self.d_k, N, -1)
                    .permute(0, 3, 1, 4, 2)
                ...
                key_w = (
                    self.key_conv1Ds_aware_temporal_context(
                        key[:, :, : self.w_length, :].permute(0, 3, 1, 2)
                    )
                    .contiguous()
                    .view(nbatches, self.h, self.d_k, N, -1)
            ...
            query = (
                self.query_conv1Ds_aware_temporal_context(query.permute(0, 3, 1, 2))[
                    :, :, :, : -self.causal_padding
                ]
                .contiguous()
                .view(nbatches, self.h, self.d_k, N, -1)
            ...
            key = (
                self.key_conv1Ds_aware_temporal_context(query.permute(0, 3, 1, 2))
                .contiguous()
                .view(nbatches, self.h, self.d_k, N, -1)
                .permute(0, 3, 1, 4, 2)
            )
...
class EncoderDecoder(nn.Module):
    ...
    def decode(self, trg, encoder_output):
        return self.prediction_generator(
            self.decoder(self.trg_embed(trg), encoder_output)
        )
    ...
    def __init__(
        ...
        if residual_connection or use_LayerNorm:
            self.sublayer = clones(
                SublayerConnection(size, dropout, residual_connection, use_LayerNorm), 2
            )
        self.size = size
        ...
        if self.residual_connection or self.use_LayerNorm:
            x = self.sublayer[0](
                ...
                lambda x: self.self_attn(
                    x, x, x, query_multi_segment=True, key_multi_segment=True
                ),
            )
...
class DecoderLayer(nn.Module):
    def __init__(
        ...
        if residual_connection or use_LayerNorm:
            self.sublayer = clones(
                SublayerConnection(size, dropout, residual_connection, use_LayerNorm), 3
            )
        ...
        if self.residual_connection or self.use_LayerNorm:
            ...
            x = self.sublayer[1](
                ...
                lambda x: self.src_attn(
                    x, m, m, query_multi_segment=False, key_multi_segment=True
                ),
            )  # output: (batch, N, T', d_model)
            return self.sublayer[2](
                x, self.feed_forward_gcn
            )  # output:  (batch, N, T', d_model)
        else:
            x = self.self_attn(
                x, x, x, tgt_mask, query_multi_segment=False, key_multi_segment=False
            )  # output: (batch, N, T', d_model)
            x = self.src_attn(
                x, m, m, query_multi_segment=False, key_multi_segment=True
            )  # output: (batch, N, T', d_model)
...
def make_model(
    ...
    norm_Adj_matrix = torch.from_numpy(norm_Adj(adj_mx)).type(
        torch.FloatTensor
    )  # 通过邻接矩阵，构造归一化的拉普拉斯矩阵
    ...
    if ScaledSAt:  # employ spatial self attention
        position_wise_gcn = PositionWiseGCNFeedForward(
            spatialAttentionScaledGCN(norm_Adj_matrix, d_model, d_model),
            dropout=dropout,
        )
    else:  # 不带attention
        position_wise_gcn = PositionWiseGCNFeedForward(
            spatialGCN(norm_Adj_matrix, d_model, d_model), dropout=dropout
        )
    ...
    spatial_position = SpatialPositionalEncoding(
        d_model,
        num_of_vertices,
        dropout,
    ...
    model = EncoderDecoder(
        encoder, decoder, encoder_embedding, decoder_embedding, generator
    )
    # param init
...
class ASTGNN(nn.Module):
    def __init__(self, configs, *args, **kwargs):
        super().__init__()
        self.enc_input_size = configs.enc_input_size
        adj_mx, distance_mx  = get_adjacency_matrix_2direction(
            configs.adj_file_path, configs.enc_in
        )
        self.model = make_model(
            configs.e_layers,
            configs.enc_input_size,
            configs.enc_input_size,
            configs.d_model,
            adj_mx,
            4,
    ...
    def forward(self, x_enc, x_dec, *args, **kwargs):
        x_enc = rearrange(x_enc, "b (l c) n -> b n l c", c=self.enc_input_size)
        x_dec = rearrange(x_dec, "b (l c) n -> b n l c", c=self.enc_input_size)
        return rearrange(self.model(x_enc, x_dec), " b n l c -> b (l c) n")
