...
在本实验的**固定 RRR 投影 + 联合端到端训练协议**下，V1（只保留方向 1）与
V2（保留方向 1+2）都没有稳定恢复完整输入 NLinear 的大部分贡献，而且方向 2
没有表现出可跨 setting、跨 seed 复现的增量价值：
...
> RRR 的前两方向能够解释独立 NLinear 平方误差任务中的大部分线性最优收益，
> 但在当前联合训练目标下，不足以作为通用的固定输入瓶颈替代完整历史。
...
（`results.csv` 的 `parameter_count` 可直接核对）。因此可以排除“参数量减少”这一解释；
实验的直接操纵变量是 **NLinear 可见的输入子空间**，由此引起的优化路径、泛化行为和
与 phase 主干的协同变化均属于该干预的下游结果。
...
| ETTm2-192 | 0.1499 / 0.2649 | 0.1545 / 0.2711 | 0.1577 / 0.2748 | 0.1576 / 0.2735 | -68.0% | -66.7% | -0.000058 | validation: retention defined |
...
ETTm2-192 的 `direct` validation MSE（0.1499）优于 `phase-only`（0.1545），
因此保留率有定义。V1/V2 的 MSE 反而高于 `phase-only`，所以保留率分别为
**-68.0% / -66.7%**；负值表示裁剪变体不仅没有保留 direct 的收益，还抵消了
phase-only 到 direct 的原始改善。
...
**这是本次实验最清晰的一条负结果：在当前固定投影与联合训练协议下，加入方向 2
没有稳定增量，并且多次得到更差的 test 结果。** 这不能进一步推出“方向 2 在数据中
没有信息”或“方向 2 本质有害”。
...
- **学习到的 gate 没有被“关掉”**。V1/V2 的 gate 都停在 0.23–0.54，说明融合层
  仍然给 NLinear 支路分配了实质性权重。该证据排除了“结果无差异只是因为支路被关闭”，
  但不能单独区分输入信息不足、有限样本泛化、优化路径变化或与 phase 主干目标不匹配。
...
- **可见方差占比极低**。V1 只看得到中心化输入方差的 0.76%–7.86%，
  ...
  这说明 RRR 的方向是“任务相关”的低能量方向，并证明“输入方差占比”不能替代
  “预测价值占比”。但低方差本身不能解释端到端退化，也不能据此断言其余方向都是噪声。
...
| 假设 | 内容 | 裁定 |
|---|---|---|
| H1 | 方向 1 是主要信息，应保留完整 NLinear 相对 phase-only 的大部分收益 | **对独立 RRR 任务成立，对固定端到端瓶颈只部分成立**。V1 的 MSE 贡献保留率中位数为 64.3%，MAE 为 46.5%；ETTh2-96 的 MSE 与 Weather-192 表现较好，其余 setting 明显不足 |
| H2 | 方向 2 提供主要的形状补充，V2 应稳定优于 V1 并接近 direct | **当前协议下不成立**。V2 仅在 1/6 setting 上双指标优于 V1，跨 seed 一致性最高只有 2/3；该结果不等价于证明方向 2 本质无信息 |
...
1. **理论 capture 与本实验的端到端保留率回答不同问题。** 计划 §3 的理论参照说前两方向能保留
   85.8%–96.7% 的可实现收益；端到端训练后实际只剩 7%–152%（中位数 61.5%）。
   RRR 优化的是独立目标 `E||y-x_last-Wz||²`；实际训练优化的是
   `Huber(y-[(1-g)·phase+g·NLinear])`。如果 phase 与 gate 固定，NLinear 对应的条件目标
   也应是 `(y-(1-g)·phase)/g`，而不是 `y-x_last`。因此前者不能直接推出后者的充分输入子空间。
2. **“任务相关方向”可以是低能量方向。** λ1 占可实现收益的 73%–86%，却只占输入
   方差的 0.8%–7.9%。可靠结论仅是“方差占比与预测价值不是同一指标”，而不是
   “低能量导致了本次失败”。
3. **V2 的表达空间包含 V1，test 变差更像优化或泛化现象。** V2 完全可以把第二方向
   权重学为零，因此它在 5/6 setting 的 MSE 上不如 V1，不是表达能力不足的证明。
   当前实验没有设置“phase/gate 冻结”或“支路独立 MSE 训练”对照，尚不能在
   信息不足、联合优化干扰和有限样本过拟合之间做因果区分。
4. **方向 2 的“形状补充”没有在当前协议下兑现。** ETTh2 两格的 λ2/λ3 gap 偏小
   可能增加方向不稳定性，但 Weather-96 的 gap 很宽仍明显退化，说明不稳定性不是
   唯一解释。
5. **结论方向明确、成本可控。** 54 次训练、6.39 GPU 小时就给出了一个六 setting、
...
- 这两项需要额外导出 test 集逐样本预测，属于计划里排在“训练完成后再补充”的
  可视化条目，不属于预注册判定的一部分；
- 本次判定由三项硬性触发条件直接给出，聚合证据已经足以完成预注册裁定；但没有
  样本级曲线时，不能判断退化主要发生在水平、局部形状、峰值还是特定变量上；
...
- **机制归因边界**：本实验没有冻结 phase/gate，也没有用独立支路 MSE 训练 V1/V2，
  因此不能把失败唯一归因为“剩余方向包含必要信息”。当前证据同样允许联合优化、
  Huber 与 RRR-MSE 目标不一致、有限样本泛化等解释。
