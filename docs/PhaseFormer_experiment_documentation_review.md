...
| **已知、决定不修** | `PhaseFormer_input_component_H1_H3_H4_stage_report_D0.md` §3.1/§3.2 采用"两行表头"（首行 5 列、子表头与数据行 8 列），GFM 下按 5 列渲染会丢掉后 3 个数值。**用户 2026-09-15 明确决定不修**，保留现状；后续不要"顺手"改这两张表，也不要重复上报。 | — |
