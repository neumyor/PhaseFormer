...
> **限定（2026-09-15）**：本节结论（"保留下来的模式仍含高频能量"）建立在"谱质心位于
> 中高段"这一**弱判据**上，最新报告已明确该判据不足：严格按周期分带后，"低频"仅在 5/7 个
> setting 成立，且主导预测方向的准确描述是"**近端指数加权的近期平均水平 → 预测区间上的
> 恒定水平位移**"，而不是"低频/局部趋势"（见
> [`PhaseFormer_rank_capacity_and_data_property_report.md`](PhaseFormer_rank_capacity_and_data_property_report.md)
> §2.3 与 §2.6 的修正声明）。本节的定性结论（压缩 ≠ 输入侧低通滤波）仍成立，但**不应**被
> 引用为"分支的有效成分是低频的"这一论据。
...
- 压缩保留下来的少数模式（encoder/decoder 基向量）本身仍含有相当比例的
  ...
  定性假设；**但实验 3 的频率判据为弱判据，见 §3.3 的限定（2026-09-15）**）。
