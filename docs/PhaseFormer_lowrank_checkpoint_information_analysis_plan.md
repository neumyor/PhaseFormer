...
### 11.8 修复后才可见的两处对照问题（如实记录，未做进一步裁定）

修复干预表之后才第一次能看到全部 10 个臂的对照。以下两点是数据本身显示的，本节只陈述
事实与限制，不给出超出数据的结论。

#### 11.8.1 `Semantic-drop` 与 `PCA-drop` 在 15/72 个 cell 上维度不同

计划 §6.2 要求"不优于随机/PCA 对照则不支持该语义机制"，因此 `Semantic-drop` 必须与
同维度的 `PCA-drop` 比较。但两者实际维度并不一致：

- `PCA-drop` 的基由 `latent_input_pca_basis(hidden, rank_dim)` 给出，**恰好 `rank_dim` 维**
  （即该 checkpoint 的秩，`q=1/4` 时最高 180）；
- `Semantic-drop` 的基是数据无关的模板字典并集的隐空间像，其秩可能**低于** `rank_dim`
  （例如 ETTh2-720 `q=1/4`：`rank=180`，语义子空间仅 36 维）。

因此 **15/72 个 cell 的两个"同维对照"维度不同**（如 semantic 36 vs pca 180、36 vs 90）。
在这些 cell 上，两者的差异无法归因于语义身份，只能归因于被删除的方向数量。

#### 11.8.2 维度相同处，两者数值完全相同

在维度相同的 57 个 cell 上，`Semantic-drop` 与 `PCA-drop` 的 `Δfused MSE` **逐 cell 精确
相等（57/57，差值恰为 0.000000）**；维度不同的 15 个 cell 上则不同（中位差 −0.0012，
其中 14 个是语义删除伤害更小，与小维度一致）。

这一条有两种可能解释，**本实验无法区分**：

1. 干预的影响主要由**被删除的方向数量**决定，与具体方向无关；
2. 或在该 rank 下语义子空间与 PCA 前 k 方向落在同一族方向上。

需要说明的是，本实验**没有设置"随机 RRR 子空间"对照**，因此不能排除解释 1；
计划 §6.2 中"不优于随机/PCA 对照"这一条，在当前证据下指向较弱的表述。

同时成立的是：`Semantic-drop` 的损害在 **60/72** 个 cell 上超出同维随机删除的 95% 区间
（`worse_than_random_95pct_fused_mse`），即"删除这些方向确实有害"这一必要性证据成立；
而 `Semantic-only` 的 `Δfused MSE` 最大仅 `+0.0027`（中位 0.000000），即"只保留该子空间
几乎完全复现原修正"这一充分性证据也成立。

无法由本实验回答的是：**这些方向之所以有效，是因为它们承载了所命名的语义，还是因为
任意同数量的主要方向都同样有效。** 表 6 与表 7 的数字不再改动，此项作为本条限制的披露。
