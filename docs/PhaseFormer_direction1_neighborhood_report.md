...
**预注册计分：部分支持（5 条判据中 4 条成立）。研究主结论：不支持“加宽方向 1
邻域是普遍有效机制”，仅保留 Weather-192 的条件性正向信号。**

- **bootstrap 切向维度承载了额外输入方差，但不能据此证明这些方向具有特殊性。**
  从 k=1 加宽到 k=8，被投影子空间承载的中心化输入方差在 7 个 setting 上单调上升：
  Electricity-336 从 12.34% 升到 47.27%，Weather-192 从 7.86% 升到 28.94%。但 Cone-k
  是嵌套子空间，增加维度后可见方差不下降是结构上预期的；本实验没有同秩随机正交子空间
  对照，因此只能说“新增切向维度承载了这些方差”，不能说这些能量是方向 1 邻域特有的。
  这些新增成分与全局 RRR 方向 2 基本不重合（多数 setting 重叠 <6%），所以它们不是方向 2
  的简单重参数化。
...
- **按数据集选择宽度不改变这个结论。** 4/4 个数据集都选了 k>1，但那是“在四个投影宽度里
  ...
  ΔMSE/ΔMAE 仍为正。
- **可以对外说的**：加宽邻域相对 Cone-1 能回收一部分投影损失，Weather-192 出现了
  MSE、MAE 与跨 seed 一致性同时成立的条件性正向信号。**不可以说的**：方向 1 周围已经
  发现稳定低维结构，或加宽方向 1 邻域是普遍改进。
...
| Setting | blocks | replicates | angle median | angle p90 | tangent numerical rank | QC |
|---|---:|---:|---:|---:|---:|---|
...
表中的 `tangent numerical rank` 是机器精度阈值下的数值秩，且受 64 个 bootstrap replicate
限制，其上限就是 64。所有 setting 取到 64 只能说明偏离矩阵在该阈值下满秩，**不能**据此
推断内在有效维度为 64，也不能证明不存在低维能量集中。事实上前 7 个切向方向已经解释
73.2%–93.4% 的偏离能量；是否构成稳定低维子空间，还需要不同 bootstrap seed/分块方案之间
的子空间一致性复核，本实验没有执行该项。

**一个重要的几何事实**：bootstrap 方向 1 与全训练集方向 1 的夹角中位数在 ETT 上是
15°–34°，只有 Weather 与 Electricity 在 6°–10°。这说明 ETT 上全训练集方向 1 对当前
区块重采样较敏感，为考察邻域提供了动机；但单次 bootstrap 云不能证明切向基跨重采样方案
稳定，也不能单独证明存在固定的低维邻域。
...
（Electricity-336 +3.6 个百分点、ETTh2-96 +1.6 个百分点）。在当前解析口径下，这些新增
切向维度呈现“承载方差较多、增加预测捕获较少”的现象，与后续端到端收益有限相符。
但由于没有同秩随机/PCA/任意正交扩展对照，不能进一步归因成“方向 1 邻域特有的低价值能量”。
...
| 预注册计分 | **部分支持（4/5）** |
| 研究主结论 | **不支持普遍机制；仅 Weather-192 保留条件性正向信号** |
...
| 子命题 | 证据 | 裁定 |
|---|---|---|
| 方向 1 周围存在稳定、低维、且带额外预测价值的局部变化空间 | 前 7 个切向方向可解释 73.2%–93.4% 的单次 bootstrap 偏离能量，但数值秩达到样本上限 64；未做跨 bootstrap seed/分块方案的子空间稳定性，也没有同秩随机扩展对照。`visible variance` 随嵌套维度增加不能单独证明邻域特殊性 | **未建立**：存在候选切向方向，但“稳定、低维、特殊且有预测价值”均未被完整证明 |
...
综合结论：**“严格秩 1 过窄”可以解释部分投影损失，但不能解释 direct 与低维投影之间的
主要差距。** 加宽固定子空间通常只是让退化收窄，没有恢复完整 NLinear；唯一双指标、3/3
seed 都成立的正向信号是 Weather-192。当前证据不支持“方向 1 周围存在一个可普遍复用的稳定
低维预测邻域”，只支持继续把 Weather-192 作为条件性案例研究。
...
2. 表 2 给出了一个待对照验证的机理线索：新增切向维度的方差增量明显大于预测捕获增量。
   在推进“换读出方式”之前，应先加入同秩随机正交扩展、全局 PCA 扩展和重复 bootstrap
   子空间稳定性对照，确认该现象是否真由方向 1 邻域构造产生。
...
- 未做同秩随机正交子空间或全局 PCA 子空间对照，因此不能判断新增方差是否为方向 1 邻域特有。
- 未用不同 bootstrap seed/区块数量重复估计切向基，因此不能声明邻域子空间本身稳定。
