...
>
> 当前状态：**仅完成方案设计，未授权正式执行实验**。待填结果表见 `docs/PhaseFormer_gold_combo_experiment_tables.md`。
...
已知超 Golden 信号的取舍如下；这保证候选不是只依据单次最优结果拼接：

| 已知信号 | 是否纳入 | 理由 |
|---|---|---|
| ETTh2-720 输出凸残差/强残差 | 是 | 新旧两轮均有约 2%–3% 双指标增益，是最稳的残差信号 |
| ETTm2-96 相位不确定性+电平+高频修正 | 是 | best-validation checkpoint 下双指标均超过 Golden |
| Electricity-336 自适应残差+MAE | 是 | 高维数据上的双指标正信号，可检验门控泛化 |
| ETTh2-96/192 自适应残差 | 间接纳入 | 机制纳入，但旧结果早于当前 checkpoint 修复协议，不作为主 setting |
| Weather-96 当前 phase policy | 暂不纳入 | MAE 增益明确，但 MSE 仅约 0.0005，接近 Golden 三位小数舍入范围 |
| Weather-192 warp/RAPE | 不纳入 | 名义增益为舍入级且单 seed，不足以支撑组合依据 |
| 纯动态相位、轨迹解码、层内/多层残差 | 不纳入 | 已有实验没有稳定超过输出端凸残差，增加组合复杂度但缺少独立收益 |
