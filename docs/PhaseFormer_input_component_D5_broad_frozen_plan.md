...
每一条件都测 full-trained 的 M0/M1/M2 从 `X` 到 remove 输入的验证 MAE/MSE 变化及 paired moving-block
95% CI。对 M1/M2 还测固定 full-input phase 预测和融合系数、只替换 NLinear-style branch 输出的反事实。
...
4. 只有 M0 接近零、M1/M2 的全模型与 NLinear-only 效应均稳定为正，才值得进入新的候选构造和多 seed
   确认。若现有15项都未满足，则停止对这一候选库的扩展，不再做高耗时重训。
