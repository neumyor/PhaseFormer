...
- 复核：ETTh2 R3≡R4 数值相等（stage A 落盘）；ETTh1/ETTh2/ETTm1/Electricity 的 Δ 均从 metrics.csv 重算核对（一致）；ETTh2 A1≡R1、A2≡R2 数值完全相等（追加轮落盘）；全部 run `resumed=false`（无断点污染，`--resume` 仅跳过已完成的 original）。
