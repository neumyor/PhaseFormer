...
此时结论应是：RRR 的前两方向能够解释独立 NLinear 平方误差任务中的大部分线性最优收益，
...
不按数据集调参。ETTm2-192 的 direct validation MSE（0.1499）优于 phase-only
（0.1545），因此保留率有定义；V1/V2 高于 phase-only，对应负保留率
（-68.0% / -66.7%）。
...
MSE 或 MAE 退化超过 2%（≥2 个）。按 §9 预设，结论为：**RRR 的前两方向能够解释独立
NLinear 平方误差任务中的大部分线性最优收益，但在当前联合训练目标下，不足以作为通用的
固定输入瓶颈替代完整历史。**
...
- V1/V2 与 direct 参数量相同，因此可以排除参数量差异；实验直接改变的是可见输入子空间，
  由此诱发的优化路径、泛化与分支协同变化仍可能共同影响结果。
