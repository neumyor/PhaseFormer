...
- **最近完成的探索（2026-09-16；预注册计分：部分支持；研究主结论：不支持普遍机制）**：
  ...
  **5 条预注册判据中 4 条成立**，但其中“选中 k>1”和“优于 RRR-2”均不要求超过 direct。
  嵌套邻域承载的可见输入方差随 k 单调上升（Electricity-336 12.34% → 47.27%），
  该单调性缺少同秩随机/PCA 扩展对照，不能证明邻域方向具有特殊性。三 seed 口径下，
