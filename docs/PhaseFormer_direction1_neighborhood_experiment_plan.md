...
> 状态：**已全量执行并回填结果；预注册计分为部分支持，研究主结论为不支持普遍机制**
>
> 执行日期：2026-09-16
>
...
> 以下五条是实验执行前定义的**预注册计分规则**，用于约束结果回填，不等同于科学机制的
> 充分证据。特别是，第 1 条只比较候选宽度，第 4 条只比较另一个受限投影 RRR-2；二者均
> 不要求 Cone-k 超过完整输入的 direct。因此最终报告必须同时给出预注册分数和相对 direct
> 的研究主结论。
...
只有部分数据集稳定选择 `k>1`，或 MSE 改善但 MAE、跨 seed 一致性不足时，预注册计分记为
“部分支持”。该标签本身不能推出方向 1 邻域具有特殊性或端到端预测价值；仍需检查相对
direct 的 MSE/MAE、跨 seed 一致性，以及随机正交/PCA 等同秩对照。

### 预注册“不支持”标签

多数数据集选择 `k=1`，或更宽邻域增加大量可见方差但没有改善 test MSE/MAE。此时结论为：

> 当前实验不支持“加宽方向 1 邻域能够稳定改善端到端预测”。如果没有同秩随机正交/PCA
> 对照，也不能把随维度增加的可见方差归因为方向 1 邻域特有能量。
...
表中的 `tangent numerical rank` 为机器精度阈值下的数值秩，并受 64 个 bootstrap replicate
限制，上限就是 64。所有 setting 取到 64 不能解释为内在有效维度为 64，也不能证明不存在
低维能量集中；前 7 个切向方向实际已解释 73.2%–93.4% 的偏离能量。由于本轮没有更换
bootstrap seed 或分块方案重复估计，不能据此声明切向子空间稳定。
...
1. **新增切向维度承载了更多可见输入方差。** 从 k=1 到 k=8，可见方差在所有 setting 上
   单调上升，Electricity-336 从 12.34% 升到 47.27%（+34.93 个百分点），Weather-192
   从 7.86% 升到 28.94%。但 Cone-k 是嵌套子空间，这种单调性本身是结构上预期的；缺少
   同秩随机正交/PCA 扩展对照，不能证明这些方差是方向 1 邻域特有的。
2. **方差增量只伴随较小的预测捕获增量。** 独立线性预测捕获在同样区间只从
   66.2%→69.8%（Electricity-336）、78.3%→86.2%（Weather-192）等小幅上升，多数 setting
   的增量在 1–5 个百分点内。这是与端到端收益有限一致的描述性证据，不足以完成机制归因。
...
| 预注册计分 | **部分支持**（5 条判据中 4 条成立；第 2 条不成立） |
...
按照 §7 的预注册计分规则，本轮为 4/5 的“部分支持”。但该计分不能替代研究主结论：
判据 1 只说明 test 选择在四个投影宽度中偏向 k>1，判据 4 只说明 Cone-k 通常不差于同样
受限的 RRR-2；二者都不要求超过 direct。实际只有 Weather-192 在三 seed 上同时改善
MSE 与 MAE；ETT、Electricity 以及 Weather-96 上，更宽邻域最多只是“退得更少”。
因此研究主结论是**不支持加宽方向 1 邻域作为普遍机制**，仅保留 Weather-192 的条件性信号。
...
5. **宽度收益的合理表述**是：bootstrap 切向扩展承载了更多输入方差，但由于子空间嵌套且
   缺少同秩随机/PCA 对照，不能证明这是方向 1 邻域特有能量。在三 seed 均值口径下，
   7 个 setting 中只有 Weather-192 的被选宽度相对 direct 同时改善 MSE 与 MAE
   （-1.594% / -1.000%），另外 6 个 setting 即使换到各自最优的 k 仍劣于 direct。
   因此本实验不支持“加宽方向 1 邻域是普遍改进”，只保留 Weather-192 的条件性正向信号。
6. **当前不能声明邻域稳定或内在低维。** 数值秩 64 受 replicate 数限制；单次 bootstrap
   的累计解释率只能说明能量集中候选，必须通过不同 bootstrap seed/区块方案的子空间对齐
   才能验证稳定性。
