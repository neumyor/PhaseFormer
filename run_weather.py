...
def main():
    ...
    for horizon in horizons:
        print(f"\n{'='*50}")
        print(f"开始Weather实验: {lookback}->{horizon}")
        print(f"{'='*50}")
    ...
    if summary_records:
        ...
        summary_csv = os.path.join("./log/training_results/PhaseFormer", f"summary_weather_{ts}.csv")
        headers = [
        ...
        try:
            ...
            print(f"✅ Weather实验汇总已导出到: {summary_csv}")
        except Exception as e:
            print(f"❌ 导出CSV失败: {e}")
