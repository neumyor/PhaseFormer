...
def main():
    ...
    for horizon in horizons:
        print(f"\n{'='*50}")
        print(f"开始Traffic实验: {lookback}->{horizon}")
        print(f"{'='*50}")
    ...
    if summary_records:
        ...
        summary_csv = os.path.join("./log/training_results/PhaseFormer", f"summary_traffic_{ts}.csv")
        headers = [
        ...
        try:
            ...
            print(f"✅ Traffic实验汇总已导出到: {summary_csv}")
        except Exception as e:
            print(f"❌ 导出CSV失败: {e}")
