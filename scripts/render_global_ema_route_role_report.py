...
def main() -> None:
    ...
    lines = [
        ...
        "### 六种连续趋势的统一提取图", "",
        "下面每张图使用同一 validation history 的 channel 0，在六个独立子图中绘制原始 X（灰）和模型分支实际接收的末点锚定 A（紫）。各子图保留独立纵轴，避免 `recent_linear` 的长斜坡掩盖其他方法。固定样本为 ETTh1 origin 1046、Weather origin 2073、ETTm1 origin 9073；它们是已有验证样本，并非由该图后验选择。", "",
        "- [ETTh1 六成分图](figures/ETTh1__six_continuous_trend_components__origin1046_channel0.png)",
        "- [Weather 六成分图](figures/Weather__six_continuous_trend_components__origin2073_channel0.png)",
        "- [ETTm1 六成分图](figures/ETTm1__six_continuous_trend_components__origin9073_channel0.png)",
        "", "所有连续成分最后均写为 `A_t=f_t-f_{719}`，以保留 NLinear 的末值锚点：", "",
        "| 成分 | 先计算的 level/轨迹 `f_t` | 当前参数 |", "|---|---|---|",
        "| `global_linear` | 对全部720步 OLS 拟合直线 | 完整窗口 |",
        "| `recent_linear` | 对最后96步 OLS 拟合直线，并把斜率外推回完整窗口 | W=96 |",
        "| `smooth_local` | 双侧 replicate-padded Gaussian 平滑 | σ=24 |",
        "| `smooth_multiscale` | `Gaussian_{24}(X)-Gaussian_{72}(X)` | σshort=24, σlong=72；这是中频差分，并非纯趋势 |",
        "| `causal_ema` | `f_0=X_0; f_t=αX_t+(1-α)f_{t-1}` | ETTh1/ETTm1 α=.006；Weather α=.024 |",
        "| `holt_local_linear` | `l_t=αX_t+(1-α)(l_{t-1}+b_{t-1})`; `b_t=β(l_t-l_{t-1})+(1-β)b_{t-1}` | ETTh1/ETTm1 α=.006,β=.0015；Weather α=.024,β=.006 |",
        "",
        "## 3. 双向选样的量化概览", "",
